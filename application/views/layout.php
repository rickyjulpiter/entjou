...
<script type="text/javascript">
    function SwalDelete(hapus){swal({title: 'Yakin mau di Hapus?',text: "Data akan terhapus permanen!",type: 'warning',showCancelButton: true,showLoaderOnConfirm: true,preConfirm: function() {return new Promise(function(resolve) {$.ajax({url: "<?=base_url().$this->uri->segment(1).'/'.$this->uri->segment(1).'_hapus'?>",type: 'POST',data: {'hapus':hapus, '<?=$this->security->get_csrf_token_name()?>' : '<?=$this->security->get_csrf_hash()?>'},dataType: 'json'}).done(function(response){swal('Berhasil dihapus!', response.message, 'success');datatb.fnDraw();}).fail(function(){swal('Oops...', 'Terjadi Kesalahan !', 'error');datatb.fnDraw();});});},allowOutsideClick: false});}$(document).ready(function(){$(document).on('click', '.hapus', function(e){var hapus = $(this).data('hapus');SwalDelete(hapus);e.preventDefault();});
    });

    function SwalDelete1(nonaktif){swal({title: 'Yakin mau di nonaktif?',text: "Kelas akan menjadi nonaktif. Jika ingin mengaktifkan kembali perbarui data kelas!",type: 'warning',showCancelButton: true,showLoaderOnConfirm: true,preConfirm: function() {return new Promise(function(resolve) {$.ajax({url: "<?=base_url().$this->uri->segment(1).'/'.$this->uri->segment(1).'_nonaktif'?>",type: 'POST',data: {'nonaktif':nonaktif, '<?=$this->security->get_csrf_token_name()?>' : '<?=$this->security->get_csrf_hash()?>'},dataType: 'json'}).done(function(response){swal('Berhasil dinonaktif!', response.message, 'success');datatb.fnDraw();}).fail(function(){swal('Oops...', 'Terjadi Kesalahan !', 'error');datatb.fnDraw();});});},allowOutsideClick: false});}$(document).ready(function(){$(document).on('click', '.nonaktif', function(e){var nonaktif = $(this).data('nonaktif');SwalDelete1(nonaktif);e.preventDefault();});
    });
    function SwalDelete2(hapus){swal({title: 'Yakin mau di Hapus?',text: "Data akan terhapus permanen!",type: 'warning',showCancelButton: true,showLoaderOnConfirm: true,preConfirm: function() {return new Promise(function(resolve) {$.ajax({url: "<?=base_url().$this->uri->segment(1).'/'.$this->uri->segment(2).'_hapus'?>",type: 'POST',data: {'hapus':hapus, '<?=$this->security->get_csrf_token_name()?>' : '<?=$this->security->get_csrf_hash()?>'},dataType: 'json'}).done(function(response){swal('Berhasil dihapus!', response.message, 'success');datatb.fnDraw();}).fail(function(){swal('Oops...', 'Terjadi Kesalahan !', 'error');datatb.fnDraw();});});},allowOutsideClick: false});}$(document).ready(function(){$(document).on('click', '.hapus2', function(e){var hapus = $(this).data('hapus2');SwalDelete2(hapus);e.preventDefault();});
    });
</script>